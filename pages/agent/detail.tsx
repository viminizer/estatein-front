...
import { serializeFetchParameter, useMutation, useQuery, useReactiveVar } from '@apollo/client';
import { useRouter } from 'next/router';
import { Property } from '../../libs/types/property/property';
import { Member } from '../../libs/types/member/member';
import { sweetErrorHandling, sweetMixinSuccessAlert, sweetTopSmallSuccessAlert } from '../../libs/sweetAlert';
...
const AgentDetail: NextPage = ({ initialInput, initialComment, ...props }: any) => {
  ...
  useEffect(() => {
    if (searchFilter.search.memberId) getPropertiesRefetch({ variables: { input: searchFilter } }).then();
  }, [searchFilter]);
  useEffect(() => {
    if (commentInquiry.search.commentRefId) getCommentsRefetch({ variables: { input: commentInquiry } }).then();
  }, [commentInquiry]);
